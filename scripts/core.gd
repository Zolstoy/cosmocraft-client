...
func leave():
	if state != State.Core.LOADING && state != State.Core.PLAYING_SOLO && state != State.Core.PLAYING_ONLINE:
		return
	back_to_welcome()
...
func play_online() -> void:
	...
	config.set_value("user", "email", ui.email_saved)
	config.set_value("user", "nickname", ui.nickname_saved)
	config.set_value("user", "password", ui.password_saved)
	config.save("user://config.cfg")
